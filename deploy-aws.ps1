# AWS Lambda deployment script for TTS Proxy (PowerShell)
$ErrorActionPreference = "Stop"

Write-Host "🚀 Deploying TTS Proxy to AWS Lambda..." -ForegroundColor Green
...
$ELEVENLABS_MODEL_ID = if ($env:ELEVENLABS_MODEL_ID) { $env:ELEVENLABS_MODEL_ID } else { "eleven_multilingual_v2" }
$VAPI_SECRET = $env:VAPI_SECRET
$DEMO_MODE = if ($env:DEMO_MODE) { $env:DEMO_MODE } else { "false" }

# Validate required environment variables
if (-not $ELEVENLABS_API_KEY -or -not $ELEVENLABS_VOICE_ID -or -not $VAPI_SECRET) {
    Write-Host "❌ Missing required environment variables:" -ForegroundColor Red
    Write-Host "   ELEVENLABS_API_KEY: $(if ($ELEVENLABS_API_KEY) { 'Set' } else { 'Not set' })" -ForegroundColor Red
    Write-Host "   ELEVENLABS_VOICE_ID: $(if ($ELEVENLABS_VOICE_ID) { 'Set' } else { 'Not set' })" -ForegroundColor Red
...
if ($EXISTING_API -and $EXISTING_API -ne "None") {
    ...
    $API_ID = aws apigatewayv2 create-api --name "elevenlabs-tts-api" --protocol-type HTTP --query 'ApiId' --output text --region $REGION
